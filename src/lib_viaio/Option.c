/*
 *  $Id: Option.c 3289 2008-05-16 14:36:36Z karstenm $
 *
 *  This file contains routines for parsing and providing help on
 ...
 *  VIdentifyFiles
 *
 *  Identify the files specified by command line arguments.
 *
 ...
 *  VParseFilterCmdX
 *
 *  Similar to VParseFilterCmd, but input filename is preserved.
 *  Thus, the parameter *inp is now a VString, not a FILE-pointer.
 *
 *  G.Lohmann, MPI-KYB, May 2017
 */

void VParseFilterCmdX (int noptions, VOptionDescRec opts[],
		      int argc, char **argv, VString *inp, FILE **outp)
{
...
/* Report usage without license information */
void VReportUsage_nl (VStringConst program, int noptions,
		   VOptionDescRec options[], VStringConst other_args)
{
  fprintf (stderr, "\nUsage: %s <options>", program);
  if (other_args)
